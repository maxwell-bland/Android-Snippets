 ...
 */
public class BtDeviceListAdapter extends DeviceListAdapter {

    public BtDeviceListAdapter(Context context, HitlistUtil hitlist) {
        super(context, android.R.layout.simple_list_item_1, new ArrayList<DeviceDatapoint>(),
                hitlist);
    }
    ...
    public View getView(int position, View convertView, @NonNull ViewGroup parent) {
        ...
        final BtDeviceDatapoint deviceDatapoint = (BtDeviceDatapoint) getItem(position);
        assert deviceDatapoint != null;
        new BtDeviceDataHolder(hitlist, convertView, deviceDatapoint);
        return convertView;
    }

    @Override
    public void updateDatapoint(DeviceDatapoint point) {
        if (point instanceof BtDeviceDatapoint) {
